...
function IntegrationsView(props: Props) {
  ...
  const [qaBackupPrefix, setQaBackupPrefix] = useState("");
  const manualOperations = props.snapshot.operations.filter(
    (operation) => operation.source === "manual",
  );
  const hasExactQaInventory =
    manualOperations.length === 2 &&
    ["Canário logística", "Operação E2E domingo"].every((name) =>
      manualOperations.some((operation) => operation.event_name === name),
    );
  ...
  const backupQa = () => {
    let result = "Backup privado criado.";
    void props.run(async () => {
      const backup = await postJson<{
        backupPrefix: string;
        counts: { operations: number; events: number; incidents: number; evidence: number };
      }>("cleanup-synthetic-qa", { mode: "backup" });
      setQaBackupPrefix(backup.backupPrefix);
      result = `Backup privado confirmado: ${backup.counts.operations} operações, ${backup.counts.events} eventos e ${backup.counts.evidence} evidências.`;
    }, () => result);
  };
  const deleteQa = () => {
    void props.run(async () => {
      await postJson("cleanup-synthetic-qa", {
        mode: "delete",
        backupPrefix: qaBackupPrefix,
      });
      setQaBackupPrefix("");
      await props.refresh();
    }, "Duas operações sintéticas removidas e reconciliadas; backup privado preservado.");
  };
  const domains = [
    [
      "Agenda e pedidos",
  ...
  return (
    <div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        ...
        {(hasExactQaInventory || manualOperations.length === 0) && <article className="rounded-xl border border-[#eadcae] bg-[#fffaf0] p-5"><h3 className="text-xl font-semibold">Limpeza dos cenários de QA</h3><p className="mt-2 text-sm text-[#705817]">Alvo fechado: duas operações manuais sintéticas, quatro eventos e suas evidências. Pessoas, equipes, frota e dados externos não entram na limpeza.</p>{hasExactQaInventory && <button type="button" onClick={backupQa} disabled={props.busy || Boolean(qaBackupPrefix)} className="mt-4 min-h-11 w-full rounded-lg border border-[#b89229] px-4 py-3 font-semibold disabled:opacity-40">Gerar backup privado recuperável</button>}<button type="button" onClick={deleteQa} disabled={props.busy || (hasExactQaInventory && !qaBackupPrefix)} className="mt-3 min-h-11 w-full rounded-lg bg-[#8b2e27] px-4 py-3 font-semibold text-white disabled:opacity-40">{hasExactQaInventory ? "Excluir somente as duas operações sintéticas" : "Concluir remoção das evidências originais"}</button>{qaBackupPrefix && <p className="mt-3 text-xs text-[#705817]">Backup remoto verificado. A exclusão permanece separada e exige ação explícita.</p>}</article>}
