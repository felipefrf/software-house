...
import { BrandHeader, Button, Card, Disclosure, Screen, StatusStrip } from "@/components/Ui";
...
export default function OperationScreen() {
  ...
  const { work, outbox, online, busy, session, message, setItemChecked, setMessage } = useApp();
    ...
    <Screen>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Card style={styles.nextCard}>
          <Text style={styles.eyebrow}>{operation.status === "completed" ? "Operação concluída" : operation.status === "cancelled" ? "Operação cancelada" : `O que fazer agora • Etapa ${currentIndex + 1} de ${operationStages.length}`}</Text>
          ...
          <Text style={styles.cardCopy}>
            {missingAssignments.length
              ...
              : "Abra esta etapa, confira as orientações e registre a foto. O app indica o que falta antes de confirmar."}
          </Text>
          {missingAssignments.length ? (
        ...
        {message ? <Text accessibilityLiveRegion="polite" style={styles.offlineNote}>{message}</Text> : null}
        ...
        {sourceItems.length ? (
          <View style={styles.manifest}>
            <View style={styles.manifestHead}>
              <View>
                <Text style={styles.manifestEyebrow}>Itens da operação</Text>
                <Text style={styles.manifestTitle}>{checkedCount} de {sourceItems.length} conferidos</Text>
                <Text style={styles.cardCopy}>Confira pela foto e toque no item para marcar.</Text>
              </View>
        ...
        <Disclosure title="Ver todas as etapas">
        <ScrollView
          ref={rail}
          horizontal
        ...
        <Text accessibilityRole="header" style={styles.manifestTitle}>Equipe e transporte</Text>
...
const styles = StyleSheet.create({
  ...
  eventName: { color: colors.ink, fontFamily: fonts.display, fontSize: 24, lineHeight: 32, fontWeight: "600" },
  destination: { color: colors.muted, fontSize: 14, lineHeight: 20, marginTop: 7 },
  schedule: { color: colors.ink, fontSize: 13, fontWeight: "700", marginTop: 9 },
  cacheAge: { color: colors.muted, fontSize: 14, marginTop: 5 },
  manifest: { marginTop: 28, paddingTop: 24, borderTopWidth: 1, borderTopColor: colors.line, gap: 10 },
  ...
  offlineNote: { color: colors.amber, fontSize: 12, lineHeight: 21, padding: 10, borderRadius: 8, backgroundColor: colors.amberSoft },
  ...
  itemCardDisabled: { opacity: 1 },
  itemCardPressed: { transform: [{ scale: 0.99 }] },
  photoPlaceholder: { width: 96, minHeight: 112, alignItems: "center", justifyContent: "center", gap: 6, padding: 8, borderRightWidth: 1, borderRightColor: colors.line, backgroundColor: colors.surface },
  photoImage: { position: "absolute", inset: 0, width: "100%", height: "100%" },
  ...
  nodeDone: { borderColor: colors.success, backgroundColor: colors.surface },
  ...
  nodeTextDone: { color: colors.success },
  ...
  nextCard: { marginTop: 24, marginBottom: 16, backgroundColor: colors.surface, borderColor: colors.line, borderLeftWidth: 4, borderLeftColor: colors.green },
  eyebrow: { color: colors.green, fontSize: 13, fontWeight: "600" },
  nextTitle: { color: colors.ink, fontFamily: fonts.display, fontSize: 22, lineHeight: 29, fontWeight: "600", marginTop: 4 },
  cardTitle: { color: colors.ink, fontSize: 19, fontWeight: "700" },
  cardCopy: { color: colors.muted, fontSize: 13, lineHeight: 23, marginTop: 6 },
